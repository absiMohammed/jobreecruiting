...
export default class JobSearchList extends React.Component {
    ...
    componentDidMount = async() => {
        ...
        try {
            ...
            let response = await fetch(Constants.urls.root+'api/job/search?user_id='+this.state.user.id, {
                method: 'GET',
                headers: {
    ...
    _onِApply = async(id) => {
        ...
        try {
            let response = await fetch(Constants.urls.root+'/api/job-user/create', {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
                    ...
                    <List
                        ...
                        renderRow={data =>
                            <ListItem thumbnail style={{ paddingLeft: 20 }}>
                                <Body>
                                <Text>{data.name}</Text>
                                <Text note numberOfLines={3}>{data.name+'\n'+data.description}</Text>
                                </Body>
                                <Right>
