...
export default class ManageSkills extends React.Component {
    ...
    componentDidMount = async() => {
        ...
        try {
            ...
            let response = await fetch(Constants.urls.root+'api/user-skill/'+this.state.user.id+'/view', {
                method: 'GET',
                headers: {
    ...
    _onDelete = async(id) => {
        ...
        try {
            let response = await fetch(Constants.urls.root+'/api/user-skill/delete/'+id, {
                method: 'DELETE',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
            ...
            <Container style={styles.container}>
                <ImageBackground source={launchscreenBg} style={styles.imageContainer}>
                    <Header transparent
                        ...
                        <Right>
                            <Button transparent onPress={() => this.props.navigation.navigate("AddSkill")}>
                                <Icon name="add" />
                            </Button>
                        </Right>
                    </Header>
                ...
                <Content padder style={{'backgroundColor': '#ffffff'}}>
                    ...
                    <List
                        ...
                        renderRow={data =>
                            ...
                            <Right>
                                <Button transparent
                                        onPress={() => this.props.navigation.navigate("EditSkill", {
                                            'id': data.id,
                        ...
                        renderLeftHiddenRow={data =>
                            <Button
                                full
                                info
                                onPress={() => this.props.navigation.navigate("ViewSkill", {
                                    'id': data.id,
                                    'description': data.description,
                                    'category': data.category.name === null ? null : data.category.name
                                })}
                                style={{
                                    flex: 1,
                                    alignItems: "center",
